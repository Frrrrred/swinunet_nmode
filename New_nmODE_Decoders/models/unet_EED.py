...
from New_nmODE_Decoders.models import EED
...
class UNet_EED(nn.Module):
    ...
    def forward(self, x):
        ...
        y4 = self.act(self.up2(x4,y5,self.delta,input_weight[1][0],input_weight[1][1],8))
        y4 = self.drop(y4)

        y3 = self.act(self.up3(x3,y4,self.delta,input_weight[2][0],input_weight[2][1],4))
        y3 = self.drop(y3)

        y2 = self.act(self.up4(x2,y3,self.delta,input_weight[3][0],input_weight[3][1],2))
        y2 = self.drop(y2)

        y1 = self.up5(x1, y2, self.delta,input_weight[4][0], input_weight[4][1],1)
        y1 = self.drop(y1)
