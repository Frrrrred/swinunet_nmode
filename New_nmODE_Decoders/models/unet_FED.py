...
from New_nmODE_Decoders.models import FED
...
class UNet_FED(nn.Module):
    def __init__(self, activefunc,droprate,kernel_size,n_channels, n_classes, bilinear=True):
        ...
        self.up2 = FED.CR_B(in_channel=512, out_channel=cfg.y0_channel, kernel_size=self.ker, stride=1, padding=self.pad)
        self.up3 = FED.CR_B(in_channel=256, out_channel=cfg.y0_channel, kernel_size=self.ker, stride=1, padding=self.pad)
        self.up4 = FED.CR_B(in_channel=128, out_channel=cfg.y0_channel, kernel_size=self.ker, stride=1, padding=self.pad)
        self.up5 = FED.CR_B(in_channel=64, out_channel=cfg.y0_channel, kernel_size=self.ker, stride=1, padding=self.pad)
    ...
    def forward(self, x):
        ...
        y5, _ = self.up1(x5, y0, self.delta, input_weight[0][0], input_weight[0][1], 16)
        y5 = self.drop(self.act(y5))

        y4, _ = self.up2(x4, y5, self.delta, input_weight[1][0], input_weight[1][1], 8)
        y4 = self.drop(self.act(y4))

        y3, _ = self.up3(x3, y4, self.delta, input_weight[2][0], input_weight[2][1], 4)
        y3 = self.drop(self.act(y3))

        y2, _ = self.up4(x2, y3, self.delta, input_weight[3][0], input_weight[3][1], 2)
...
if __name__ == '__main__':
    net = UNet_FED(activefunc='relu',droprate=0.1,kernel_size=3,n_channels=3, n_classes=1).cuda()
