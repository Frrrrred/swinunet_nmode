...
from New_nmODE_Decoders.models import LMD, EED
...
class UNet_LMD(nn.Module):
    ...
    def forward(self, x):
        ...
        y5 = self.act(self.up1(x5,y0,self.delta,input_weight[0][0],input_weight[0][1],16))
        y5 = self.drop(y5)

        y4 = self.act(self.up2(x4,y0,y5,self.delta,input_weight[1][0],input_weight[1][1],input_weight[1][2],8))
        y4 = self.drop(y4)

        y3 = self.act(self.up3(x3,y5,y4,self.delta,input_weight[2][0],input_weight[2][1],input_weight[2][2],4))
        y3 = self.drop(y3)

        y2 = self.act(self.up4(x2,y4,y3,self.delta,input_weight[3][0],input_weight[3][1],input_weight[3][2],2))
        y2 = self.drop(y2)

        y1 = self.up5(x1, y3, y2, self.delta,input_weight[4][0], input_weight[4][1],input_weight[4][2],1)
        y1 = self.drop(y1)
