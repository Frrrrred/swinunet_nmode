...
def get_optimizer(config, model,lr):
    assert config.opt in ['Adadelta', 'Adagrad', 'Adam', 'AdamW', 'Adamax', 'ASGD', 'RMSprop', 'Rprop', 'SGD'], \
    ...
    if config.opt == 'Adadelta':
        ...
        return torch.optim.AdamW(
            [param for name, param in model.named_parameters() if name != 'delta'],  # 原model.named_parameters()
...
class myRandomRotation:
    def __init__(self, p=0.5, degree=None):
        if degree is None:
            degree = [0, 360]
