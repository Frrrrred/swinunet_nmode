...
def save_checkpoint(config, epoch, model, max_accuracy, min_loss, optimizer, lr_scheduler, logger, now_saves):
    ...
    # now_saves是一个列表，保留最新五个checkpoint的路径和当前的max_accuracy值
    now_saves.append((save_path, max_accuracy))
    if len(now_saves) > 5:
        logger.info(f"{now_saves[0][0]} removed !!!")
        os.remove(now_saves[0][0])
        now_saves.pop(0)

    now_saves.sort(key=lambda x: x[1], reverse=False)
...
def load_pretrained(config, model, logger, is_diffent):
